...
def mock_run(monkeypatch):
    ...
    monkeypatch.setattr(
        handler,
        "_find_memsearch",
        lambda: [sys.executable, "-m", "memsearch"],
    )
...
def test_injects_top2_as_context(mock_run, tmp_path):
    ...
    args = calls[0]
    assert args[:3] == [sys.executable, "-m", "memsearch"]
    assert args[3:4] == ["search"]
    assert args[4] == "how does watch work?"
