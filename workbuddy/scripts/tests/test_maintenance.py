...
def test_prompt_defaults_point_to_vendored_prompts(mock_pkg, tmp_path):
    _, cfg = mock_pkg
    handler._maintenance(str(tmp_path), str(tmp_path / ".memsearch"))
    for task in ("project_review", "user_profile", "memory_to_skill"):
        p = getattr(cfg.prompts, task)
        assert p.endswith(os.path.join("prompts", task + ".txt"))
        assert os.path.isfile(p)  # 仓内 prompts/ 真实存在
