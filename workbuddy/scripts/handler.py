...
def _find_memsearch():
    """后端定位：仅认 pip/conda 装进当前解释器的 memsearch 包（python -m 入口）。

    返回 [sys.executable, "-m", "memsearch"] 或 None。不用 uvx / PATH 上的
    memsearch.exe——console-subsystem shim 在无控制台父进程下会弹黑窗，且
    uvx 环境与解释器 site-packages 可能分裂。
    """
    try:
        return (
            [sys.executable, "-m", "memsearch"]
            if importlib.util.find_spec("memsearch")
            else None
        )
    except Exception:
        return None
...
def _maintenance(project_dir, memsearch_dir):
    ...
    try:
        ...
        return 1
    cfg = resolve_config()
    # prompt 缺省指向仓内 prompts/（逐字复制自上游 plugins/_shared/prompts，MIT）
    prompts_dir = os.path.abspath(os.path.join(SCRIPT_DIR, "..", "prompts"))
    for task in ("project_review", "user_profile", "memory_to_skill"):
        if not getattr(cfg.prompts, task, ""):
            f = os.path.join(prompts_dir, task + ".txt")
            if os.path.isfile(f):
